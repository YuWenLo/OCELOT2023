...
def arg_parser():
    ...
    parser.add_argument('--mode', type=str, default='cell', help='data mode:cell/tissue')
...
def save_mask(model, test_loader, tta=False):
    model.eval()
    FPS = AvgMeter()
    pbar = enumerate(test_loader)
    pbar = tqdm(pbar, total=len(test_loader))
    save_path = "/work/wagw1014/OCELOT/tissue/tta_all_tissue_strloss_650e_prediction_indice/"#'/work/wagw1014/OCELOT/tissue/prediction_indice/'
...
if __name__ == '__main__':
    ...
    for k in range(opt.kfold):
        ...
        train_dataset = create_dataset(opt.data_path, opt.trainsize, opt.augmentation, True, opt.dataratio, opt.rect, k=k, k_fold=opt.kfold, seed=opt.seed, num_class=opt.class_num, mode=opt.mode)
        train_loader = data.DataLoader(dataset=train_dataset, batch_size=opt.batchsize, shuffle=True, num_workers=4, pin_memory=True)
        del train_dataset

        test_dataset = create_dataset(opt.data_path, opt.trainsize, False, False, opt.dataratio, opt.rect, k=k, k_fold=opt.kfold, seed=opt.seed, num_class=opt.class_num)
        ...
        if opt.test:
            ...
            save_mask(model, test_loader, opt.tta)
