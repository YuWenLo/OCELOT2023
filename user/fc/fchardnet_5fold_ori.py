...
class PytorchaFcCellModel():
    ...
    def __init__(self, metadata):
        ...
        # cell
        self.cell_hardnet = hardnet(n_classes=self.n_classes)
        ...
        # tissue
        self.tissue_hardnet = hardnet(n_classes=self.n_classes)
    ...
    def find_cells(self, heatmap):
        ...
        peaks = feature.peak_local_max(
            arr, min_distance=10, exclude_border=0, threshold_abs=0.97
        ) # List[y, x]
    ...
    def __call__(self, cell_patch, tissue_patch, pair_id):
        ...
        # cell
        _curr_path = os.path.split(__file__)[0]
        weights_dir = os.path.join(_curr_path, 'checkpoints/all5fold_cell_1c_r5/')
        weights = sorted(os.listdir(weights_dir))
        logits = None
        for k, weight in enumerate(weights):
            # print('weight = ', weight)
            self.load_checkpoint(mode='cell', weight_path = os.path.join('checkpoints/all5fold_cell_1c_r5/', weight))
            self.cell_hardnet = self.cell_hardnet.to(self.device)
            self.cell_hardnet.eval()
        ...
        cell_predictions = self.find_cells(heatmap)
        
        # tissue
        tissue_logits = self.tissue_hardnet(tissue_patch)
        tissue_logits = (tissue_logits + tf.vflip(tf.hflip(self.tissue_hardnet(tf.vflip(tf.hflip(tissue_patch))))))/2
